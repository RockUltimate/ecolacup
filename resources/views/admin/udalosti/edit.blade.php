    ...
    <x-slot name="header">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div class="space-y-3">
                ...
                <p class="max-w-3xl text-sm leading-6 text-on-surface-variant dark:text-[#c3c8bb]">Úprava základních údajů, disciplín a možností ustájení z jednoho místa.</p>
            </div>
            ...
            <section class="grid gap-6 lg:grid-cols-2">
                <div class="panel space-y-5 p-6 sm:p-8">
                    <div>
                        ...
                        <h2 class="mt-2 text-2xl text-on-surface dark:text-[#e5e2dd]">Přidat nebo odebrat</h2>
                    </div>
                    ...
                    <form method="POST" action="{{ route('admin.udalosti.moznosti.store', $udalost) }}" class="grid gap-4 md:grid-cols-2">
                        ...
                        <label class="md:col-span-2 flex items-center gap-3 rounded-[1rem] border border-[#eadfcc] bg-white/60 px-4 py-3 text-sm text-on-surface dark:text-[#e5e2dd]">
                        ...
                        @forelse($udalost->moznosti as $moznost)
                            <div class="surface-muted flex items-start justify-between gap-4">
                                <div>
                                    ...
                                    <p class="mt-1 text-sm text-on-surface-variant dark:text-[#c3c8bb]">{{ number_format((float) $moznost->cena, 2, ',', ' ') }} Kč @if($moznost->min_vek)• min. věk {{ $moznost->min_vek }}@endif</p>
                                </div>
                                <form method="POST" action="{{ route('admin.udalosti.moznosti.destroy', [$udalost, $moznost]) }}">
                                    @csrf
                                    @method('DELETE')
                                    <button class="text-sm text-error underline underline-offset-4">Smazat</button>
                                </form>
                            </div>
                        @empty
                            <p class="text-sm text-on-surface-variant dark:text-[#c3c8bb]">Zatím bez disciplín.</p>
                        @endforelse
                    </div>
                </div>
                    ...
                    <div class="space-y-3">
                        @forelse($udalost->ustajeniMoznosti as $moznost)
                            <div class="surface-muted flex items-start justify-between gap-4">
                                <div>
                                    <p class="font-semibold text-on-surface dark:text-[#e5e2dd]">{{ $moznost->nazev }}</p>
                                    <p class="mt-1 text-sm text-on-surface-variant dark:text-[#c3c8bb]">{{ $moznost->typ }} • {{ number_format((float) $moznost->cena, 2, ',', ' ') }} Kč @if($moznost->kapacita)• kapacita {{ $moznost->kapacita }}@endif</p>
                                </div>
                        ...
                        @empty
                            <p class="text-sm text-on-surface-variant dark:text-[#c3c8bb]">Zatím bez možností ustájení.</p>
                        @endforelse
                    </div>
                </div>
            </section>
